...
const totalSum = (input) => {
  // Solution code here...
  let num = 0
  let outer = input.map( inner => {
    let adds = inner.map ( value => {
      let sum = num+=value
      return sum
    })
    return adds
...
const divisibleByFiveTwoToThePower = (input) => {
  ...
  let arr = [];
  let arr1 = input.map(powerTwo => {
    let smallArr = powerTwo.map(byFive => {
      if (byFive % 5 === 0 ) {
        return Math.pow(2,byFive)
      }
      if (byFive % 5 !== 0 || byFive!== typeof num ){
        return byFive = []
      }
...
let findMaleAndFemale = (data) => {
  ...
  data.forEach(character =>{
    if ((character.gender ==='female') || (character.gender ==='male')) {
      return (characterNames === '' ? characterNames = character.name : characterNames = characterNames + ' and ' + character.name);
    }
  });
